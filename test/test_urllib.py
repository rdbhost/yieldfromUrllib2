...
def urlopen(url, data=None, proxies=None):
    ...
    if proxies is not None:
        opener = request.FancyURLopener(proxies=proxies)
    elif not _urlopener:
        ...
        opener = _urlopener
    if data is None:
        _r = yield from opener.open(url)
    else:
        _r = yield from opener.open(url, data)
    return _r
...
class FakeFTPMixin(object):
    def fakeftp(self):
        ...
        self._ftpwrapper_class = request.ftpwrapper
        request.ftpwrapper = FakeFtpWrapper

    def unfakeftp(self):
        request.ftpwrapper = self._ftpwrapper_class
...
class urlopen_FileTests(unittest.TestCase):
    ...
    def _setUp(self):
        ...
        self.returned_obj = yield from urlopen("file:%s" % self.pathname)
    ...
    def test_relativelocalfile(self):
        yield from self._setUp()
        self.aioAssertRaises(ValueError, request.urlopen, './' + self.pathname)
...
class ProxyTests(unittest.TestCase):
    ...
    def test_getproxies_environment_keep_no_proxies(self):
        ...
        self.assertTrue(request.proxy_bypass_environment('anotherdomain.com'))
...
class urlopen_HttpTests(unittest.TestCase): #, FakeHTTPMixin, FakeFTPMixin):
    ...
    def check_read(self, ver):
        ...
        body = b"HTTP/" + ver + b" 200 OK\r\n\r\nHello!"

        def _run(host, port):
            url = '%s:%s' % (host, port)
            fp = yield from urlopen(url)
            self.assertEqual((yield from fp.readline()), b"Hello!")
            self.assertEqual((yield from fp.readline()), b"")
            self.assertEqual(fp.geturl(), 'http://python.org/')
            self.assertEqual(fp.getcode(), 200)

        _run_with_server(_run, body=body)

    @async_test
    def test_url_fragment(self):
        # Issue #11703: geturl() omits fragments in the original URL.
        url = 'http://httpbin.org/ip#OK'
        #self.fakehttp(b"HTTP/1.1 200 OK\r\n\r\nHello!")
        fp = yield from request.urlopen(url)
    ...
    def test_read_0_9(self):
        ...
        # a status line)
        yield from self.check_read(b"0.9")

    def test_read_1_0(self):
        yield from self.check_read(b"1.0")

    def test_read_1_1(self):
        yield from self.check_read(b"1.1")

    @async_test
    def test_read_bogus(self):
        # urlopen() should raise OSError for many error codes.
        body = (b'''HTTP/1.1 401 Authentication Required
...
''')
        try:
            self.aioAssertRaises(OSError, urlopen, "http://python.org/")
        finally:
            pass
            # self.unfakehttp()
...
''')
        try:
            self.aioAssertRaises(error.HTTPError, urlopen,
                              "http://python.org/")
        finally:
            pass
            # self.unfakehttp()
    ...
    def test_empty_socket(self):
        ...
        #self.fakehttp(b'')
        try:
            self.aioAssertRaises(OSError, urlopen, "http://something")
        finally:
            pass
            # self.unfakehttp()
    ...
    def test_missing_localfile(self):
        # Test for #10836
        with self.assertRaises(error.URLError) as e:
            yield from urlopen('file://localhost/a/file/which/doesnot/exists.py')
    ...
    def test_file_notexists(self):
        ...
        try:
            ...
            tfu = yield from urlopen(tmp_fileurl)
            with tfu as fobj:
        ...
        with self.assertRaises(error.URLError):
            yield from urlopen(tmp_fileurl)

    @async_test
    def test_ftp_nohost(self):
        test_ftp_url = 'ftp:///path'
        with self.assertRaises(error.URLError) as e:
            yield from urlopen(test_ftp_url)
    ...
    def test_ftp_nonexisting(self):
        with self.assertRaises(error.URLError) as e:
            yield from urlopen('ftp://localhost/a/file/which/doesnot/exists.py')
        self.assertFalse(e.exception.filename)
        self.assertTrue(e.exception.reason)

    @patch.object(request, 'MAXFTPCACHE', 0)
    @async_test
    def tst_ftp_cache_pruning(self):
        #self.fakeftp()
        try:
            request.ftpcache['test'] = request.ftpwrapper('user', 'pass', 'localhost', 21, [])
            yield from urlopen('ftp://localhost')
        finally:
            #self.unfakeftp()
            pass
    ...
    def test_userpass_inurl(self):
        fp = yield from urlopen("http://user:pass@httpbin.org/")
        self.assertEqual(fp.geturl(), 'http://user:pass@httpbin.org/')
        self.assertEqual(fp.getcode(), 200)
    ...
    def test_userpass_inurl_w_spaces(self):
        ...
        userpass = "a b:c d"
        url = "http://{}@httpbin.org/".format(userpass)
...
class urlopen_DataTests(unittest.TestCase):
    ...
    def _setUp(self):
        ...
        self.text_url_resp = yield from request.urlopen(self.text_url)
        self.text_url_base64_resp = yield from request.urlopen(
            self.text_url_base64)
        self.image_url_resp = yield from request.urlopen(self.image_url)
    ...
    def test_info(self):
        ...
        self.assertEqual(self.image_url_resp.info()['content-length'],
            str(len(self.image)))
        _r = yield from request.urlopen("data:,")
        self.assertEqual(_r.info().get_params(), [('text/plain', ''), ('charset', 'US-ASCII')])
    ...
    def test_missing_comma(self):
        ...
        yield from self._setUp()
        self.aioAssertRaises(ValueError, request.urlopen, 'data:text/plain')
    ...
    def test_invalid_base64_data(self):
        ...
        self.aioAssertRaises(ValueError, request.urlopen, 'data:;base64,Cg=')
...
class urlretrieve_FileTests(unittest.TestCase):
    ...
    def constructLocalFileUrl(self, filePath):
        ...
        return "file://%s" % request.pathname2url(filePath)
    ...
    def test_basic(self):
        ...
        result = yield from request.urlretrieve("file:%s" % support.TESTFN)
    ...
    def test_copy(self):
        ...
        result = yield from request.urlretrieve(self.constructLocalFileUrl(
    ...
    def test_reporthook_0_bytes(self):
        ...
        yield from request.urlretrieve(self.constructLocalFileUrl(srcFileName),
...
''')
        ...
        def _run(host, port):
            ...
            with self.assertRaises(error.ContentTooShortError):
                yield from request.urlretrieve(url, reporthook=_reporthook)
...
''')
        def _run(host, port):
            url = 'http://%s:%s' % (host, port)
            with self.assertRaises(error.ContentTooShortError):
                yield from request.urlretrieve(url)
...
class QuotingTests(unittest.TestCase):
    ...
    def test_never_quote(self):
        ...
        do_not_quote = '' .join(["ABCDEFGHIJKLMNOPQRSTUVWXYZ",
                                 ...
                                 "0123456789",
                                 "_.-"])
        result = parse.quote(do_not_quote)
        self.assertEqual(do_not_quote, result,
                         "using quote(): %r != %r" % (do_not_quote, result))
        result = parse.quote_plus(do_not_quote)
        self.assertEqual(do_not_quote, result,
    ...
    def test_default_safe(self):
        ...
        self.assertEqual(parse.quote.__defaults__[0], '/')

    @async_test
    def test_safe(self):
        # Test setting 'safe' parameter does what it should do
        quote_by_default = "<>"
        result = parse.quote(quote_by_default, safe=quote_by_default)
        self.assertEqual(quote_by_default, result,
                         "using quote(): %r != %r" % (quote_by_default, result))
        result = parse.quote_plus(quote_by_default,
                                         safe=quote_by_default)
        self.assertEqual(quote_by_default, result,
                         "using quote_plus(): %r != %r" %
                         (quote_by_default, result))
        # Safe expressed as bytes rather than str
        result = parse.quote(quote_by_default, safe=b"<>")
        self.assertEqual(quote_by_default, result,
                         "using quote(): %r != %r" % (quote_by_default, result))
        # "Safe" non-ASCII characters should have no effect
        # (Since URIs are not allowed to have non-ASCII characters)
        result = parse.quote("a\xfcb", encoding="latin-1", safe="\xfc")
        expect = parse.quote("a\xfcb", encoding="latin-1", safe="")
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" %
                         (expect, result))
        # Same as above, but using a bytes rather than str
        result = parse.quote("a\xfcb", encoding="latin-1", safe=b"\xfc")
        expect = parse.quote("a\xfcb", encoding="latin-1", safe="")
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" %
                         (expect, result))
    ...
    def test_default_quoting(self):
        ...
        for char in should_quote:
            result = parse.quote(char)
            self.assertEqual(hexescape(char), result,
                             "using quote(): "
                             "%s should be escaped to %s, not %s" %
                             (char, hexescape(char), result))
            result = parse.quote_plus(char)
            self.assertEqual(hexescape(char), result,
                             "using quote_plus(): "
        ...
        partial_quote = "ab[]cd"
        expected = "ab%5B%5Dcd"
        result = parse.quote(partial_quote)
        self.assertEqual(expected, result,
                         "using quote(): %r != %r" % (expected, result))
        result = parse.quote_plus(partial_quote)
        self.assertEqual(expected, result,
    ...
    def test_quoting_space(self):
        ...
        result = parse.quote_plus(' ')
        self.assertEqual(result, '+',
                         "using quote_plus(): %r != +" % result)
        given = "a b cd e f"
        expect = given.replace(' ', hexescape(' '))
        result = parse.quote(given)
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        expect = given.replace(' ', '+')
        result = parse.quote_plus(given)
        self.assertEqual(expect, result,
                         "using quote_plus(): %r != %r" % (expect, result))

    @async_test
    def test_quoting_plus(self):
        self.assertEqual(parse.quote_plus('alpha+beta gamma'),
                         'alpha%2Bbeta+gamma')
        self.assertEqual(parse.quote_plus('alpha+beta gamma', '+'),
                         'alpha+beta+gamma')
        # Test with bytes
        self.assertEqual(parse.quote_plus(b'alpha+beta gamma'),
                         'alpha%2Bbeta+gamma')
        # Test with safe bytes
        self.assertEqual(parse.quote_plus('alpha+beta gamma', b'+'),
                         'alpha+beta+gamma')
    ...
    def test_quote_bytes(self):
        ...
        self.assertRaises(TypeError, parse.quote, given,
                            encoding="latin-1")
        # quote_from_bytes should work the same
        result = parse.quote_from_bytes(given)
        self.assertEqual(expect, result,
    ...
    def test_quote_with_unicode(self):
        ...
        result = parse.quote(given, encoding=None, errors=None)
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        # Characters in Latin-1 range, encoded with Latin-1
        given = "\xa2\xd8ab\xff"
        expect = "%A2%D8ab%FF"
        result = parse.quote(given, encoding="latin-1")
        self.assertEqual(expect, result,
                         "using quote(): %r != %r" % (expect, result))
        ...
        given = "\u6f22\u5b57"
        self.assertRaises(UnicodeEncodeError, parse.quote, given,
                                    encoding="latin-1")
    ...
    def test_quote_plus_with_unicode(self):
        ...
        given = "\xa2\xd8 \xff"
        expect = "%A2%D8+%FF"
        result = parse.quote_plus(given, encoding="latin-1")
        self.assertEqual(expect, result,
                         "using quote_plus(): %r != %r" % (expect, result))
...
class UnquotingTests(unittest.TestCase):
    ...
    def test_unquoting(self):
        ...
        for num in range(128):
            ...
            expect = chr(num)
            result = parse.unquote(given)
            self.assertEqual(expect, result,
                             "using unquote(): %r != %r" % (expect, result))
            result = parse.unquote_plus(given)
            self.assertEqual(expect, result,
        ...
        del escape_list
        result = parse.unquote(escape_string)
        self.assertEqual(result.count('%'), 1,
                         "using unquote(): not all characters escaped: "
                         "%s" % result)
        self.assertRaises((TypeError, AttributeError), parse.unquote, None)
        self.assertRaises((TypeError, AttributeError), parse.unquote, ())
        with support.check_warnings(('', BytesWarning), quiet=True):
            self.assertRaises((TypeError, AttributeError), parse.unquote, b'')
    ...
    def test_unquoting_badpercent(self):
        ...
        result = parse.unquote_to_bytes(given)
        ...
        self.assertEqual(expect, result, "using unquote_to_bytes(): %r != %r"
                         % (expect, result))
        self.assertRaises((TypeError, AttributeError), parse.unquote_to_bytes, None)
        self.assertRaises((TypeError, AttributeError), parse.unquote_to_bytes, ())
    ...
    def test_unquote_to_bytes(self):
        ...
        # encoded).
        result = parse.unquote_to_bytes("\u6f22%C3%BC")
    ...
    def test_unquote_with_unicode(self):
        ...
        result = parse.unquote(given, encoding=None, errors=None)
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))

        # Characters in the Latin-1 range, encoded with Latin-1
        result = parse.unquote('br%FCckner_sapporo_20050930.doc',
                                      encoding="latin-1")
        expect = 'br\u00fcckner_sapporo_20050930.doc'
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
        ...
        result = parse.unquote(given, errors="replace")
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
        ...
        given = "%F3%B1"
        expect = ""
        result = parse.unquote(given, errors="ignore")
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))

        # A mix of non-ASCII and percent-encoded characters, UTF-8
        result = parse.unquote("\u6f22%C3%BC")
        expect = '\u6f22\u00fc'
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
        ...
        result = parse.unquote("\u6f22%FC", encoding="latin-1")
        expect = '\u6f22\u00fc'
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
...
class urlencode_Tests(unittest.TestCase):
    ...
    def test_doseq(self):
        ...
        expect = "sequence=%s" % parse.quote_plus(str(['1', '2', '3']))
        result = parse.urlencode(given)
        self.assertEqual(expect, result)
        result = parse.urlencode(given, True)
    ...
    def test_nonstring_values(self):
        self.assertEqual("a=1", parse.urlencode({"a": 1}))
        self.assertEqual("a=None", parse.urlencode({"a": None}))

    def test_nonstring_seq_values(self):
        self.assertEqual("a=1&a=2", parse.urlencode({"a": [1, 2]}, True))
        self.assertEqual("a=None&a=a",
                         parse.urlencode({"a": [None, "a"]}, True))
        data = collections.OrderedDict([("a", 1), ("b", 1)])
        self.assertEqual("a=a&a=b",
                         parse.urlencode({"a": data}, True))

    def test_urlencode_encoding(self):
        # ASCII encoding. Expect %3F with errors="replace'
        given = (('\u00a0', '\u00c1'),)
        expect = '%3F=%3F'
        result = parse.urlencode(given, encoding="ASCII", errors="replace")
        self.assertEqual(expect, result)
        ...
        given = (('\u00a0', '\u00c1'),)
        expect = '%A0=%C1'
        result = parse.urlencode(given, encoding="latin-1")
        self.assertEqual(expect, result)
    ...
    def test_urlencode_encoding_doseq(self):
        ...
        given = (('\u00a0', '\u00c1'),)
        expect = '%3F=%3F'
        result = parse.urlencode(given, doseq=True,
        ...
        given = (("\u00a0", "\u00c1"),)
        expect = '%A0=%C1'
        result = parse.urlencode(given, True, encoding="latin-1")
        self.assertEqual(expect, result)
    ...
    def test_urlencode_encoding_safe_parameter(self):
        ...
        given = ((b'\xa0\x24', b'\xc1\x24'),)
        result = parse.urlencode(given, safe=":$")
        expect = '%A0$=%C1$'
        self.assertEqual(expect, result)

        given = ((b'\xa0\x24', b'\xc1\x24'),)
        result = parse.urlencode(given, doseq=True, safe=":$")
        expect = '%A0$=%C1$'
        self.assertEqual(expect, result)
        ...
        result = parse.urlencode(given, True, safe=":$")
        self.assertEqual(expect, result)
...
class Pathname_Tests(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        result = request.pathname2url(expected_path)
        self.assertEqual(expected_url, result,
                         "pathname2url() failed; %s != %s" %
                         (result, expected_url))
        result = request.url2pathname(expected_url)
    ...
    def test_quoting(self):
        ...
        expect = "needs/%s/here" % parse.quote("quot=ing")
        result = request.pathname2url(given)
        self.assertEqual(expect, result,
                         "pathname2url() failed; %s != %s" %
                         (expect, result))
        expect = given
        result = request.url2pathname(result)
        self.assertEqual(expect, result,
                         "url2pathname() failed; %s != %s" %
                         (expect, result))
        given = os.path.join("make sure", "using_quote")
        expect = "%s/using_quote" % parse.quote("make sure")
        result = request.pathname2url(given)
        self.assertEqual(expect, result,
                         "pathname2url() failed; %s != %s" %
                         (expect, result))
        ...
        result = request.url2pathname(given)
        self.assertEqual(expect, result,
                         "url2pathname() failed; %s != %s" %
                         (expect, result))
    ...
    def test_ntpath(self):
        ...
        expect = 'C:\\'
        for url in given:
            result = request.url2pathname(url)
            self.assertEqual(expect, result,
                             'request..url2pathname() failed; %s != %s' %
                             (expect, result))
        given = '///C|/path'
        expect = 'C:\\path'
        result = request.url2pathname(given)
...
class Utility_Tests(unittest.TestCase):
    ...
    def test_splitpasswd(self):
        ...
        """
        self.assertEqual(('user', 'ab'), parse.splitpasswd('user:ab'))
        self.assertEqual(('user', 'a\nb'), parse.splitpasswd('user:a\nb'))
        self.assertEqual(('user', 'a\tb'), parse.splitpasswd('user:a\tb'))
        self.assertEqual(('user', 'a\rb'), parse.splitpasswd('user:a\rb'))
        self.assertEqual(('user', 'a\fb'), parse.splitpasswd('user:a\fb'))
        self.assertEqual(('user', 'a\vb'), parse.splitpasswd('user:a\vb'))
        self.assertEqual(('user', 'a:b'), parse.splitpasswd('user:a:b'))
        self.assertEqual(('user', 'a b'), parse.splitpasswd('user:a b'))
        self.assertEqual(('user 2', 'ab'), parse.splitpasswd('user 2:ab'))
        self.assertEqual(('user+1', 'a+b'), parse.splitpasswd('user+1:a+b'))

    def test_thishost(self):
        """Test the request.thishost utility function returns a tuple"""
        self.assertIsInstance(request.thishost(), tuple)
...
class URLopener_Tests(unittest.TestCase):
    ...
    def test_quoted_open(self):
        class DummyURLopener(request.URLopener):
            @asyncio.coroutine
        ...
        with support.check_warnings(
                ...
                DeprecationWarning)):
            duo = DummyURLopener()
            _r = yield from duo.open('spam://example/ /')
            self.assertEqual(_r, '//example/%20/')

            # test the safe characters are not quoted by urlopen
            _r = yield from duo.open("spam://c:|windows%/:=&?~#+!$,;'@()*[]|/path/")
            self.assertEqual(_r, "//c:|windows%/:=&?~#+!$,;'@()*[]|/path/")
...
class RequestTests(unittest.TestCase):
    """Unit tests for request.Request."""
    ...
    def test_with_method_arg(self):
        Request = request.Request
        req = Request("http://www.python.org", method='HEAD')
        self.assertEqual(req.method, 'HEAD')
        self.assertEqual(req.get_method(), 'HEAD')
        req = Request("http://www.python.org", {}, method='HEAD')
        self.assertEqual(req.method, 'HEAD')
        self.assertEqual(req.get_method(), 'HEAD')
        req = Request("http://www.python.org", method='GET')
        self.assertEqual(req.get_method(), 'GET')
        req.method = 'HEAD'
        self.assertEqual(req.get_method(), 'HEAD')
