...
from request import Request, OpenerDirector, _parse_proxy, _proxy_bypass_macosx_sysconf
#from urllib.parse import urlparse
from parse import urlparse, urljoin
#import urllib.error
import error
...
class TrivialTests(unittest.TestCase):
    ...
    def test_trivial(self):
        ...
        self.aioAssertRaises(ValueError, request.urlopen, 'bogus url')

        # XXX Name hacking to get this to work on Windows.
        fname = os.path.abspath(request.__file__).replace('\\', '/')
        ...
        f = yield from request.urlopen(file_url)
    ...
    def test_parse_http_list(self):
        ...
        for string, list in tests:
            self.assertEqual(request.parse_http_list(string), list)

    def test_URLError_reasonstr(self):
        err = error.URLError('reason')
...
class MockHTTPHandler(request.BaseHandler):
...
class MockHTTPSHandler(request.AbstractHTTPHandler):
    # Useful for testing the Proxy-Authorization request by verifying the
    # properties of httpcon

    def __init__(self):
        request.AbstractHTTPHandler.__init__(self)
...
class OpenerDirectorTests(unittest.TestCase):
    ...
    def test_badly_named_methods(self):
        ...
        o.add_handler(request.UnknownHandler())
        for scheme in "do", "proxy", "redirect":
            self.aioAssertRaises(URLError, o.open, scheme+"://example.com/")
    ...
    def test_handler_order(self):
        ...
        yield from o.open("http://example.com/")
...
def sanepathname2url(path):
    ...
    urlpath = request.pathname2url(path)
...
class HandlerTests(unittest.TestCase):
    ...
    def test_ftp(self):
        ...
        class NullFTPHandler(request.FTPHandler):
            def __init__(self, data): self.data = data
    ...
    def test_file(self):
        ...
        for url in urls:
            ...
            try:
                ...
                r = yield from h.file_open(Request(url))
                try:
                    data = r.read()
                    headers = r.info()
                    respurl = r.geturl()
...
# XXXX bug: fails with OSError, should be URLError
    ...
    def test_http(self):
        ...
        for method, data in [("GET", None), ("POST", b"blah")]:
            ...
            http = MockHTTPClass()
            r = yield from h.do_open(http, req)
        ...
        self.aioAssertRaises(error.URLError, h.do_open, http, req)

        # Check for TypeError on POST data which is str.
        req = Request("http://example.com/","badpost")
        self.aioAssertRaises(TypeError, h.do_request_, req)
    ...
    def test_redirect(self):
        ...
        try:
            ...
            self.assertEqual(count, request.HTTPRedirectHandler.max_repeats)
    ...
    def test_cookie_redirect(self):
        ...
        hdeh = request.HTTPDefaultErrorHandler()
        hrh = request.HTTPRedirectHandler()
        cp = request.HTTPCookieProcessor(cj)
        o = build_test_opener(hh, hdeh, hrh, cp)
        yield from o.open("http://www.example.com/")
    ...
    def test_redirect_fragment(self):
        ...
        fp = yield from o.open('http://www.example.com')
        self.assertEqual(fp.geturl(), redirected_url.strip())

    @async_test
    def test_proxy(self):
        o = OpenerDirector()
        ph = request.ProxyHandler(dict(http="proxy.example.com:3128"))
        o.add_handler(ph)
        meth_spec = [
    ...
    def test_proxy_no_proxy(self):
        ...
        o = OpenerDirector()
        ph = request.ProxyHandler(dict(http="proxy.example.com"))
        o.add_handler(ph)
    ...
    def test_proxy_https(self):
        o = OpenerDirector()
        ph = request.ProxyHandler(dict(https="proxy.example.com:3128"))
        o.add_handler(ph)
        meth_spec = [
    ...
    def test_basic_auth(self, quote_char='"'):
        ...
        yield from self._test_basic_auth(opener, auth_handler, "Authorization",
    ...
    def test_proxy_basic_auth(self):
        ...
        opener.add_handler(auth_handler)
        opener.add_handler(http_handler)
        yield from self._test_basic_auth(opener, auth_handler, "Proxy-authorization",
                              realm, http_handler, password_manager,
    ...
    def test_basic_and_digest_auth_handlers(self):
        ...
        yield from self._test_basic_auth(opener, basic_handler, "Authorization",
                              realm, http_handler, password_manager,
                              "http://acme.example.com/protected",
                              "http://acme.example.com/protected",
                              )
    ...
    def test_unsupported_auth_digest_handler(self):
        ...
        digest_auth_handler = request.HTTPDigestAuthHandler(None)
        http_handler = MockHTTPHandler(
            401, 'WWW-Authenticate: Kerberos\r\n\r\n')
        opener.add_handler(digest_auth_handler)
        opener.add_handler(http_handler)
        self.aioAssertRaises(ValueError, opener.open, "http://www.example.com")

    def test_unsupported_auth_basic_handler(self):
        # While using BasicAuthHandler
        opener = OpenerDirector()
        basic_auth_handler = request.HTTPBasicAuthHandler(None)
        http_handler = MockHTTPHandler(
    ...
    def _test_basic_auth(self, opener, auth_handler, auth_header,
        ...
        yield from opener.open(request_url)
...
class MiscTests(unittest.TestCase):
    ...
    def test_build_opener(self):
        class MyHTTPHandler(request.HTTPHandler): pass
        class FooHandler(request.BaseHandler):
            def foo_open(self): pass
        class BarHandler(request.BaseHandler):
        ...
        # in explicitly
        o = build_opener()
        self.opener_has_handler(o, request.HTTPHandler)
        o = build_opener(request.HTTPHandler)
        self.opener_has_handler(o, request.HTTPHandler)
        o = build_opener(request.HTTPHandler())
        self.opener_has_handler(o, request.HTTPHandler)

        # Issue2670: multiple handlers sharing the same base class
        class MyOtherHTTPHandler(request.HTTPHandler): pass
    ...
    def test_HTTPError_interface(self):
        ...
        err = error.HTTPError(url, code, msg, hdrs, fp)
...
def test_main(verbose=None):
    ...
    support.run_doctest(request, verbose)
    tests = (TrivialTests,
             OpenerDirectorTests,
             HandlerTests,
             MiscTests,
