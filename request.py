...
def urlopen(url, data=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT,
    ...
    if cafile or capath or cadefault:
        ...
        opener = _opener
    _o = yield from opener.open(url, data, timeout)
    return _o
...
def urlretrieve(url, filename=None, reporthook=None, data=None):
    ...
    _u = yield from urlopen(url, data)
    with contextlib.closing(_u) as fp:
        headers = fp.info()
        ...
        with tfp:
            ...
            while True:
                if type(fp) in (asyncio.StreamReader, client.HTTPResponse):
                    block = yield from fp.read(bs)
                else:
                    block = fp.read(bs)
                if not block:
                    break
...
class OpenerDirector:
    ...
    def open(self, fullurl, data=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
        ...
        for processor in self.process_response.get(protocol, []):
            meth = getattr(processor, meth_name)
            response = yield from meth(req, response)
    ...
    def error(self, proto, *args):
        ...
        result = yield from self._call_chain(*args)
        if result:
            return result

        if http_err:
            args = (dict, 'default', 'http_error_default') + orig_args
            ret = yield from self._call_chain(*args)
            return ret
...
class HTTPErrorProcessor(BaseHandler):
    ...
    def http_response(self, request, response):
        ...
        if not (200 <= code < 300):
            response = yield from self.parent.error(
...
class AbstractHTTPHandler(BaseHandler):
    ...
    def do_open(self, http_class, req, **http_conn_args):
        ...
        http_class must implement the HTTPConnection API from client.
        """
        host = req.host
        if not host:
            raise URLError('no host given')
...
class HTTPHandler(AbstractHTTPHandler):
    ...
    def http_open(self, req):
        _o = yield from self.do_open(client.HTTPConnection, req)
        return _o
...
if hasattr(client, 'HTTPSConnection'):
    ...
    class HTTPSHandler(AbstractHTTPHandler):
        ...
        def https_open(self, req):
            _r = yield from self.do_open(client.HTTPSConnection, req,
...
class FileHandler(BaseHandler):
    ...
    def file_open(self, req):
        ...
        if url[:2] == '//' and url[2:3] != '/' and (req.host and
            ...
            ret = yield from self.open_local_file(req)
            return ret
...
class URLopener:
    ...
    def open(self, fullurl, data=None):
        ...
        try:
            if data is None:
                _r = yield from (getattr(self, name)(url))
            else:
                _r = yield from (getattr(self, name)(url, data))
            return _r
    ...
    def retrieve(self, url, filename=None, reporthook=None, data=None):
        ...
        if filename is None and (not type or type == 'file'):
            try:
                fp = yield from self.open_local_file(url1)
                hdrs = fp.info()
                fp.close()
                return url2pathname(splithost(url1)[1]), hdrs
            except OSError as msg:
                pass
        fp = yield from self.open(url, data)
        try:
            headers = fp.info()
            if filename:
                tfp = open(filename, 'wb')
    ...
    def _open_generic_http(self, connection_factory, url, data):
        ...
        if data is not None:
            ...
            yield from http_conn.request("POST", selector, data, headers)
        else:
            yield from http_conn.request("GET", selector, headers=headers)

        try:
            response = yield from http_conn.getresponse()
    ...
    def open_http(self, url, data=None):
        """Use HTTP protocol."""
        _o = yield from self._open_generic_http(client.HTTPConnection, url, data)
        return _o
    ...
    def http_error(self, url, fp, errcode, errmsg, headers, data=None):
        ...
        if hasattr(self, name):
            ...
            if data is None:
                result = yield from method(url, fp, errcode, errmsg, headers)
            else:
                result = yield from method(url, fp, errcode, errmsg, headers, data)
    ...
    if _have_ssl:
        def _https_connection(self, host):
            return client.HTTPSConnection(host,
        ...
        def open_https(self, url, data=None):
            """Use HTTPS protocol."""
            _r = yield from self._open_generic_http(self._https_connection, url, data)
            return _r
    ...
    def open_file(self, url):
        ...
        if url[:2] == '//' and url[2:3] != '/' and url[2:12].lower() != 'localhost/':
            ...
            _r = yield from self.open_local_file(url)
            return _r

    @asyncio.coroutine
    def open_local_file(self, url):
        """Use local file."""
        yield None # make a generator
        import email.utils
        import mimetypes
